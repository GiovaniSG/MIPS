# Calcula diversos números de Fibonacci,coloca em um vetor e imprime
.data
fibs:.word   0 : 19         # "array" de words para conter os fib values
size: .word  19             # tamanho do "array" (agrees with array declaration)
...
loop: lw   $s3, 0($s0)      # Get value from array F[n-2]
      ...
      bgtz $s1, loop        # repeat while not finished (Branch on greater than zero)
...
print:add  $t0, $zero, $a0  # starting address of array of data to be printed
      ...
      li   $v0, 4           # specify Print String service
      syscall               # print the heading string (to use syscall, first set $v0 with the code of the function you want to call, then use syscall)
      			    #$v0=4 (print string) 
